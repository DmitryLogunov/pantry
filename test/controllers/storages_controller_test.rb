...
class StoragesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create storage" do
    assert_difference('Storage.count') do
      post storages_url, params: { storage: { description: @storage.description, title: @storage.title } }
    end
  ...
  test "should update storage" do
    patch storage_url(@storage), params: { storage: { description: @storage.description, title: @storage.title } }
